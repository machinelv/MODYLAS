...
!!         (EoM) for anisotropic NPT ensemble.
!>
!----------------------------------------------------------------------
!>
!! \brief  Module for NPT ensemble (anisotropic).
...
!----------------------------------------------------------------------
!>
!! \brief  Subroutine to integrate EoM for npt ensemble (anisotropic)
!! \author Yoshimichi Andoh
!<
  subroutine npt_pr_integrate()
!----------------------------------------------------------------------
    use atom_mass
...
!default: MTD
    ...
    !     ### NHCA (XO) ###
    call nhc_pr_thermo()
    call update_vboxg_outermost_pr(dtL*maxMTm*maxMTl)
...
!$omp end parallel
          ...
          if(scaleM==1d0)then
             call update_vboxg_outer_pr(dtL*maxMTm)
             call update_velocities_outer(dtL*maxMTm)
          endif

          call update_vboxg_inner_pr(dtL)
...
!$omp end parallel
    ...
    call calc_MeanForce
#endif
  end subroutine npt_pr_integrate
!----------------------------------------------------------------------
!
...
!! \author Yoshimichi Andoh
!<
  subroutine calc_hamiltonian_npt_pr()
!----------------------------------------------------------------------
    use atom_virial
    use md_const
    use md_monitors
    ...
    !NI-->
    ! Average of cell matrix (box)
    sum_box=sum_box+box
    !<--NI

  end subroutine calc_hamiltonian_npt_pr

end module npt_pr_mod
