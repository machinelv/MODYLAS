...
!!         (EoM) for isotropic NPT ensemble.
!<
!----------------------------------------------------------------------
!>
!! \brief  Module for NPT ensemble (isotropic).
...
!----------------------------------------------------------------------
!>
!! \brief  Subroutine to integrate EoM for npt ensemble (isotropic)
!! \author Yoshimichi Andoh
!<
  subroutine npt_a_integrate()
!----------------------------------------------------------------------
    use atom_mass
...
#include "timing.h90"
    ...
    TIME_STOP(TM_PRE_MTLOOP)

!default: MTD
    if(mdstep==0)then
          TIME_BARRIER(TMB_COMM_DIRECT2_FSHORT)
          TIME_START(TM_COMM_DIRECT2_FSHORT)
      call recover_fshort()
          TIME_STOP(TM_COMM_DIRECT2_FSHORT)
          TIME_BARRIER(TMB_COMM_DIRECT2_FMIDDL)
          TIME_START(TM_COMM_DIRECT2_FMIDDL)
      call recover_fmiddle()
          TIME_STOP(TM_COMM_DIRECT2_FMIDDL)
    endif

    !     ### NHCA (XO) ###
    TIME_START(TM_THERMO)
    call nhc_a_thermo()
    TIME_STOP(TM_THERMO)
    TIME_START(TM_PRE_MTLOOP)
    call update_vboxg_outermost(dtL*maxMTm*maxMTl)
...
!$omp end parallel
          ...
          TIME_STOP(TM_MTLOOP_SEGPREP)
10        continue
          TIME_START(TM_MTLOOP_SHAKEITR_LOCAL)
          if(totnconst > 0) then
...
!$omp end parallel
          ...
          if(scaleM==1d0)then
             call update_vboxg_outer(dtL*maxMTm)
             call update_velocities_outer(dtL*maxMTm)
          endif

          call update_vboxg_inner(dtL)
          ...
          call update_wkvir()
          TIME_STOP(TM_MTLOOP_SHAKEITR_LOCAL)
          if (totnconst .gt. 0) then
             TIME_START(TM_SHAKE_ROLL)
             call shake_roll(dtL)
             TIME_STOP(TM_SHAKE_ROLL)
             TIME_START(TM_MTLOOP_SHAKEITR_LOCAL)
             call update_wkdvir()
             TIME_STOP(TM_MTLOOP_SHAKEITR_LOCAL)
          ...
          if(MTl==maxMTl.and.MTm==maxMTm)then
             call update_wsegc()
             TIME_BARRIER(TMB_COMM_BOUND)
             TIME_START(TM_COMM_BOUND)
             call comm_bound()
             TIME_STOP(TM_COMM_BOUND)
          ...
          TIME_START(TM_MTLOOP_SHAKEITR_POST)
#ifdef SEGSHAKE
          call  calc_center_of_mass
          call  shake_com(dtL)
#endif
          TIME_STOP(TM_MTLOOP_SHAKEITR_POST)

          TIME_BARRIER(TMB_COMM_DIRECT3)
          TIME_START(TM_COMM_DIRECT3)
!default: MTD
          call comm_direct_3_dr(wkxyz)
          TIME_STOP(TM_COMM_DIRECT3)
          TIME_START(TM_APPL_PBC)
          call apply_pbc()
          TIME_STOP(TM_APPL_PBC)
...
!$omp end parallel
          ...
          TIME_STOP(TM_MTLOOP_SEGPREP2)

20        continue

          TIME_START(TM_MTLOOP_RATTLEITR_LOCAL)
          if(totnconst > 0) then
             if(liter.gt.maxliter) then
...
!$omp end parallel
          ...
          call update_wkvir()
          TIME_STOP(TM_MTLOOP_RATTLEITR_LOCAL)
          if (totnconst .gt. 0) then
             TIME_START(TM_RATTLE_ROLL)
             call rattle_roll(dtL)
             TIME_STOP(TM_RATTLE_ROLL)
             call update_wkdvir()
          endif
    ...
    call remove_system_momentum_para  !! c.o.
#ifdef SEGSHAKE
    call calc_MeanForce
#endif

  end subroutine npt_a_integrate
...
!! \author Yoshimichi Andoh
!<
  subroutine calc_hamiltonian_npt_a()
!----------------------------------------------------------------------
    use atom_virial
    use md_const
    use md_monitors
  ...
  end subroutine calc_hamiltonian_npt_a

!----------------------------------------------------------------------
!>
!! \brief  Subroutine to recover fshort in NPT ensemble.
!! \author Yoshimichi Andoh
!<
  subroutine recover_fshort
  use md_multiplestep
  use subcell
  use dr_cntl
  use comm_direct2_dr, only : comm_direct_2_dr
  implicit none
  integer(4) icx,icy,i0,k0

  call comm_direct_2_dr(fshort) 

  end subroutine recover_fshort

!----------------------------------------------------------------------
!>
!! \brief  Subroutine to recover fmiddle in NPT ensemble.
!! \author Yoshimichi Andoh
!<
  subroutine recover_fmiddle
  use md_multiplestep
  use subcell
  use dr_cntl
  use comm_direct2_dr, only : comm_direct_2_dr
  ...
  call comm_direct_2_dr(fmiddle) 

  end subroutine recover_fmiddle

end module npt_a_mod
